...
best_alternatives <- function (network_study, locations = "all")
{
    ...
    ## limit to the desired locations
    network.data <- filter (network.data, location %in% locations)

    ## for each location get a dataframe with the fastest wired and wifi networks and assemble an aggregate frame
    ldply (split (network.data, network.data$location, drop = TRUE), get_fastest_wired_and_wifi) %>%
    select (-.id, -wifi)
} 

get_fastest_wired_and_wifi <- function (network.data)
{  
    network.data <- arrange (network.data, desc(received.mbps))
    rows <- select (network.data, wifi) %>%
            unique () %>%
            row.names() %>%
            as.integer()
    slice (network.data, rows)
}
